...
// ── SUPABASE ──────────────────────────────────────────────────
// ── NEON DATABASE ─────────────────────────────────────────────
const NEON_URL = "https://console.neon.tech/api/v2/projects/lucky-wind-69757935/branches/br-plain-mode-ax7lsoh6/query";
const NEON_KEY = "npg_he5sBjSlupE6";
const NEON_CONN = "postgresql://neondb_owner:npg_he5sBjSlupE6@ep-broad-unit-axe7oe7a-pooler.c-4.us-east-2.aws.neon.tech/neondb";

async function neonQuery(sql, params=[]) {
  const r = await fetch(NEON_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${NEON_KEY}` },
    body: JSON.stringify({ query: sql, params, database_name: "neondb" })
  });
  return await r.json();
}
...
async function sbGet() {
  try {
    const r = await neonQuery("SELECT students, visitors FROM wem_data WHERE id = 'main'");
    const row = r.rows?.[0];
    return { students: row?.students || [], visitors: row?.visitors || [] };
  } catch(e) { return { students: [], visitors: [] }; }
}

async function sbSet(students, visitors) {
  try {
    const s = JSON.stringify(students).replace(/'/g, "''");
    const v = JSON.stringify(visitors).replace(/'/g, "''");
    await neonQuery(`UPDATE wem_data SET students = '${s}'::jsonb, visitors = '${v}'::jsonb, updated_at = NOW() WHERE id = 'main'`);
